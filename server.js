...
// GPT로 이미지 프롬프트 생성 (Physical Data)
async function generatePromptPhysical(move, exercise, stand, steps, distance) {
...
// GPT로 이미지 프롬프트 생성 (Mood Data)
async function generatePromptMood(timestamp, mood, note) {
    const fetch = await import('node-fetch').then(mod => mod.default); 
    const messages = [
        {
            role: "system",
            content: `You are an assistant that creates images to support self-reflection by interpreting personal data. Given the user's mood tracking data, create an image that reflects your interpretation of this data using DALL-E. Your goal is to generate a creative and original image prompt for DALL-E to produce an image. The output should only be the image prompt.`
        },
        {
            role: "user",
            content: `You will be provided with the personal data related to the mood of a day, including timestamp: ${timestamp}, mood: ${mood}, and note: ${note}.`
        },
        {
            role: "system",
            content: `
            ...
            2. Do not include any numbers, letters, or text in the drawing.
            3. Generate an image that is not easily associated with the data provided, ensuring creativity and originality.
            4. When creating keywords, reflect the interpretation of the data rather than the type of data itself. For example, interpret the described mood creatively instead of simply noting "mood". The image should embody these keywords in a creative manner. Do not include numbers, letters, or text in the drawing. Do not draw objects that directly describe “mood”, “state of mind”, and “personal data” itself.
            5. Upon regeneration, ensure the new image is based on different subjects or keywords from the previous one.
            6. The subject, mood, texture, and style of the image should vary with each generation to maintain diversity and creativity.
            7. Draw in one of the following styles: painting, photo, sketch, cartoon, impressionist, abstract, renaissance.
            `
...
async function logGenerationDetails(data, prompt, imageFilepath) {
    const logData = {
...
app.post('/generate-image-physical', async (req, res) => {
    const { move, exercise, stand, steps, distance } = req.body;

    try {
        const generatedPrompt = await generatePromptPhysical(move, exercise, stand, steps, distance);
        const fetch = await import('node-fetch').then(mod => mod.default); // 동적 import 사용
...
app.post('/generate-image-mood', async (req, res) => {
    const { timestamp, mood, note } = req.body;

    try {
        const generatedPrompt = await generatePromptMood(timestamp, mood, note);
        const fetch = await import('node-fetch').then(mod => mod.default); // 동적 import 사용
        const response = await fetch('https://api.openai.com/v1/images/generations', {
            method: 'POST',
            headers: {
        ...
        if (response.ok) {
            ...
            await saveImageToFile(imageUrls[0], imageFilepath);
            await logGenerationDetails({ timestamp, mood, note }, generatedPrompt, `/images/${imageFilename}`);
